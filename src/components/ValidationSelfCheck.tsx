...
export function ValidationSelfCheck({
  ...
  if (done) {
    ...
    return (
      <div className="rounded-2xl border border-[hsl(220,10%,18%)] bg-[hsl(220,13%,10%)] p-6 sm:p-8 text-center shadow-xl">
        {allZero ? (
          <>
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-[hsl(145,60%,45%,0.15)] border border-[hsl(145,60%,45%,0.3)] text-xl mb-3">
              👏
            </div>
            <h3 className="text-lg sm:text-xl font-bold text-[hsl(145,60%,55%)] font-[family-name:var(--font-space-grotesk)] mb-2">
              You&apos;ve done your homework 👏
            </h3>
            <p className="text-[hsl(40,8%,65%)] text-sm max-w-md mx-auto leading-relaxed mb-5 font-[family-name:var(--font-inter)]">
          ...
          <>
            <p className="text-xs font-[family-name:var(--font-mono)] uppercase tracking-wider text-[hsl(40,8%,50%)] mb-1">
              Your idea is most exposed to:
            </p>
            <h3 className="text-2xl sm:text-3xl font-bold text-[hsl(42,95%,55%)] mb-2 font-[family-name:var(--font-space-grotesk)]">
        ...
        <button
          type="button"
          onClick={handleReset}
          className="text-xs text-[hsl(40,8%,50%)] hover:text-[hsl(40,20%,90%)] transition-colors font-[family-name:var(--font-mono)] underline underline-offset-4 cursor-pointer"
        >
          ↺ Retake self-check
        </button>
      </div>
  ...
  return (
    <div className="rounded-2xl border border-[hsl(220,10%,18%)] bg-[hsl(220,13%,10%)] p-5 sm:p-7 shadow-xl">
      <div className="flex justify-between items-center mb-4 pb-3 border-b border-[hsl(220,10%,16%)]">
        <span className="text-xs text-[hsl(40,8%,50%)] font-[family-name:var(--font-mono)] font-medium">
          ...
          {QUESTIONS.map((_, i) => (
            <span
              ...
              className={`h-1.5 w-6 sm:w-8 rounded-full transition-colors duration-300 ${
                i <= step ? 'bg-[hsl(42,95%,55%)]' : 'bg-[hsl(220,10%,20%)]'
              }`}
            />
          ))}
        </div>
      ...
      <h3 className="text-base sm:text-lg font-bold text-[hsl(40,20%,94%)] mb-4 font-[family-name:var(--font-space-grotesk)]">
        {q.text}
      </h3>
      ...
      <div className="flex flex-col gap-2.5">
        {q.options.map((opt) => (
          <button
            ...
            type="button"
            onClick={() => answer(opt)}
            className="text-left px-4 py-3 rounded-xl border border-[hsl(220,10%,20%)] bg-[hsl(220,14%,8%)] text-xs sm:text-sm text-[hsl(40,20%,85%)] hover:border-[hsl(42,95%,55%)] hover:text-[hsl(40,20%,95%)] hover:bg-[hsl(220,15%,12%)] transition-all font-[family-name:var(--font-inter)] cursor-pointer flex items-center justify-between group"
          >
            <span>{opt.label}</span>
            <span className="text-[hsl(42,95%,55%)] opacity-0 group-hover:opacity-100 transition-opacity font-bold">
              →
            </span>
          </button>
        ))}
