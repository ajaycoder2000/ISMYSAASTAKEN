...
export function extractUserName(email: string, name?: string): string {
...
export function getGapTagHtml(tag: string): string {
  switch (tag) {
    case 'open_gap':
...
export function renderCustomWeeklyReportTemplate({
  gaps,
  subscriber,
  issueNumber,
  siteUrl,
  ...
  const userName = extractUserName(subscriber.email, subscriber.name);
...
<body style="margin: 0; padding: 0; background-color: #0c0e12; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #e0e0e0; -webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%; mso-line-height-rule: exactly;">
  ...
  <div style="display: none; font-size: 1px; color: #0c0e12; line-height: 1px; max-height: 0; overflow: hidden; mso-hide: all;">
    This week: ${realGaps.length} open wedges in AI workflows, DevTools &amp; micro-SaaS &rarr;
